...
\begin{itemize}
\item \textbf{Stratification} (\S\ref{sec:hierarchy}): $\LFnd \xrightarrow{\mathrm{Cls}} \LCls \supsetneq \LClsMax \xrightarrow{\mathrm{Gen}} \LAbs = \emptyset$ --- the strata are cut out by type-distinct condition packages ((R1)--(R5) on formal systems vs.\ (M1)--(M5) on classifiers, Proposition~\ref{prop:level-structure}~(ii)); the sole inclusion is $\LClsMax \subsetneq \LCls$.
...
\item \textbf{Intensional grading of the slice fibers} (Definition~\ref{def:display-grade}, Proposition~\ref{prop:grading}): each fiber $\mathrm{Int}([F])$ carries a canonical functorial $\mathbb{N}$-filtration by display grade; extensionality is exactly grade-blindness (Corollary~\ref{cor:grade-collapse}).
...
Every theorem below is proved \emph{from} a specific package of assumptions, and the paper's headline claims (``$\LAbs = \emptyset$'', ``$\Meta_{\mathrm{Cls}}^{\top}$ is categorical'') are exactly as strong as that package. Because downstream work builds on MSFS as a base --- in particular, any extension that positions itself as a \emph{limiting} formal theory must either discharge or explicitly inherit each entry --- I collect the assumptions here in one register, each with its formal status. Nothing in this subsection is new mathematics; it is the paper's trusted surface made enumerable.

\begin{description}
\item[\textbf{(A1) Foundational base.}] $\ZFC$ + two strongly inaccessible cardinals $\kappa_1 < \kappa_2$ (Convention~\ref{conv:zfc-inacc}). \emph{Status: foundational axiom package.} All results are theorems of this metatheory; by G\"odel~II the package cannot certify its own consistency, and the paper nowhere claims otherwise (cf.\ open question (Q4), \S\ref{sec:consequences}). Consistency-strength statements downstream can at best be equiconsistencies with (A1).
...
\item[\textbf{(A6) Meta-classifier axioms.}] The classifying apparatus satisfies (M1)--(M5) and, for the maximal class, (Max-1)--(Max-4) --- including the joint-conservativity clause of (Max-4) on the pair $(\Cl_{\mathbf{F}}, \mathbb{I}_{\mathbf{F}})$, which powers the faithfulness step of Theorem~\ref{thm:meta-cat}. \emph{Status: axioms on the classifier, motivated but not derived.}
...
\item[\textbf{(A8) Minimal display class.}] Intensional structure is measured against the \emph{canonical minimal} display class $\mathcal{D}_{\mathbf{F}}$, inductively generated by (D1)--(D4), and the display grade of \S\ref{sec:intensional-grading} is relative to this generation. \emph{Status: canonical-minimality convention;} a larger display class would coarsen the grading but not disturb the extensional quotient.
\end{description}

\begin{remark}[Interface for limiting extensions]\label{rem:register-interface}
The register is designed as a formal interface. An extension of MSFS claiming any form of \emph{limit} status can be audited entry-by-entry: (A1) can only be inherited (equiconsistency is the ceiling); (A2) and (A5) can be \emph{internalized} --- proved equivalent to structural trade-offs inside the extension --- but not escaped; (A3), (A4), (A7), (A8) are conventions an extension must either adopt verbatim or re-justify; (A6) may be strengthened. In particular, a limiting extension is consistent with AFN-T precisely when its limit claim is indexed by this register --- maximality \emph{within} $\LClsMax$-type strata --- rather than membership in $\LAbs$, which Theorem~\ref{thm:afnt-alpha} leaves empty.
\end{remark}
...
\item \emph{(R3)-failure with (R1), (R2), (R4), (R5) holding}: an \emph{inconsistent} r.e.\ extension of Q, e.g.\ $T_3 := \mathsf{Q} + (0 = 1)$. Over the ambient metatheory, (R3) is \emph{equivalent} to consistency for theories satisfying (R2): by the completeness theorem any consistent r.e.\ theory has a countable model, which lies in $\mathbf{U}_1 \subseteq \mathbf{U}_2$; conversely a model witnesses consistency. Hence no \emph{consistent} theory can serve as an (R3)-witness, and $T_3$ is the canonical failure mode. The remaining conditions hold for $T_3$: (R1), (R2), (R4) are syntactic and immediate; for (R5), the syntactic category $\Syn(T_3)$ is the terminal (provably-collapsed) structure, $\Mod(T_3) = \emptyset$, and the Lambek--Scott adjunction between them holds trivially (the universally-quantified clauses of (R5b) are satisfied vacuously, and the unit condition holds for the collapsed initial object). Thus $T_3$ isolates (R3) cleanly.
...
\begin{proof}
A universe-polymorphic structure $X = \prod_\ell F(\ell)$ is a section of the Grothendieck fibration $\pi_{\mathrm{End}} : \int \mathrm{End}(\cU) \to \mathrm{Level}$ associated (Lurie HTT~\cite{LurieHTT} \S 3.2) to the \emph{endofunctor diagram} $\ell \mapsto \Fun(\cU_\ell, \cU_\ell)$, with transition maps given by restriction along the universe inclusions $\cU_\ell \hookrightarrow \cU_{\ell'}$. Sections of this fibration correspond to compatible families $\{F(\ell)\}_\ell$, and the space of such sections is
...
The slice-locality theorem presents each fiber $\mathrm{Int}([F])$ as a bare $2$-category. The inductive construction of the canonical display class in Theorem~\ref{thm:I-existence} (Step 1) in fact endows every fiber with finer, functorial structure: an $\mathbb{N}$-indexed grading measuring \emph{how far from an equivalence} a display datum is. This subsection records the grading and its two structural properties; both proofs are short because the generating induction of Theorem~\ref{thm:I-existence} already carries them implicitly.

\begin{definition}[Display grade]\label{def:display-grade}
Let $F \in \cF$ and let $\cD_F = \bigcup_{n < \omega} \cD_F^{(n)}$ be the canonical minimal display class with its generating filtration (Theorem~\ref{thm:I-existence}, Step 1). For $d \in \cD_F$ define the \emph{grade}
\[ \gr(d) := \min\{\, n : d \in \cD_F^{(n)} \,\} \in \mathbb{N}. \]
Thus $\gr(d) = 0$ iff $d$ is a $1$-equivalence, and $\gr(d) = n+1$ iff $d$ first arises at stage $n+1$, i.e.\ as a $2$-pullback, a composite, or a $2$-cell transport of data of grade $\leq n$, but is not itself of grade $\leq n$. For an object $\II(F) = (\cC_F, \cD_F, \iota_F) \in \Sint$, write $\mathrm{Int}^{(n)}([F]) \subseteq \mathrm{Int}([F])$ for the full sub-$2$-category of the fiber (Theorem~\ref{thm:slice-locality}(b)) on display data of grade $\leq n$.
\end{definition}

\begin{proposition}[The grading is structural]\label{prop:grading}
\begin{enumerate}[label=(\alph*)]
\item \emph{Exhaustive filtration}: $\mathrm{Int}^{(0)}([F]) \subseteq \mathrm{Int}^{(1)}([F]) \subseteq \cdots$ with $\mathrm{Int}([F]) = \colim_{n} \mathrm{Int}^{(n)}([F])$.
\item \emph{Functoriality (grade non-increase)}: for every $1$-morphism $f : F_1 \to F_2$ in $\cF$, the pullback component $f^*|_{\cD}$ of $\II(f)$ satisfies $\gr(f^* d) \leq \gr(d)$ for all $d \in \cD_{F_2}$; hence $\II(f)$ restricts to each grading stage, $\II(f) : \mathrm{Int}^{(n)}([F_2]) \to \mathrm{Int}^{(n)}([F_1])$.
\item \emph{Subadditivity}: $\gr(d_2 \circ d_1) \leq \max(\gr(d_1), \gr(d_2)) + 1$, and $\gr(\phi^* d) \leq \gr(d) + 1$ for $2$-cell transports $\phi$.
\end{enumerate}
\end{proposition}

\begin{proof}
(a) is immediate from $\cD_F = \bigcup_n \cD_F^{(n)}$. (b) is the content of Theorem~\ref{thm:I-existence}, Step 2: the preservation of display data under $f^*$ is proved there \emph{by induction on the class generation} --- the base class ($1$-equivalences, grade $0$) is preserved by pullback, and each inductive closure step is preserved stage-by-stage by the pullback-of-pullback property; reading that induction with its stage index retained yields $f^*(\cD_{F_2}^{(n)}) \subseteq \cD_{F_1}^{(n)}$ for every $n$, which is the claim. (c) is the definition of the generating step: composites and $2$-cell transports of data available at stage $\max(\gr(d_1), \gr(d_2))$ (resp.\ $\gr(d)$) are enrolled at the next stage.
\end{proof}

\begin{corollary}[Extensionality is grade-blindness]\label{cor:grade-collapse}
The Morita $2$-localization of (I-4) inverts exactly the grade-$0$ layer: the localizing class $\mathcal{W}_\cU$ consists of morphisms whose underlying $2$-functor is an equivalence, and on display data these are the grade-$0$ elements. Consequently the extensional quotient $\Sint[\mathcal{W}_\cU^{-1}] \simeq \fM$ identifies all grades: the composite $\mathrm{Int}^{(n)}([F]) \hookrightarrow \mathrm{Int}([F]) \to \{[F]\}$ is constant in $n$. The intensional content of a foundation is precisely the tower $\{\mathrm{Int}^{(n)}([F])\}_{n}$ that the extensional projection forgets.
\end{corollary}

\begin{remark}[Reading, and an open question]\label{rem:grading-open}
The grading gives the slice fibers of Theorem~\ref{thm:slice-locality} the structure that the extensional-collapse example (Theorem~\ref{thm:I-existence}, Step 7) implicitly exploits: the typing invariant $\tau$ is computed on display data of \emph{bounded grade}, and the $\MLTT$/$\ETT$ distinction is visible already at finite stages of the tower. Two natural questions remain open: (i) \emph{strictness} --- for which $F$ is the filtration $\mathrm{Int}^{(n)}([F])$ strictly increasing in $n$ (no grade collapse at a finite stage)? A collapse at stage $n_0$ would express a strong normal-form property of $F$'s display geometry; (ii) \emph{grade as a Morita-refinement invariant} --- whether the minimal collapse stage $n_0(F) \in \mathbb{N} \cup \{\infty\}$, which is invariant under $\Sint$-equivalence by Proposition~\ref{prop:grading}(b), separates Morita-equivalent foundations beyond the binary invariant $\tau$ of Step 7, providing a quantitative refinement scale between extensional identity and intensional distinctness.
\end{remark}
...
\item \textbf{(Max-4) Intensional completeness with conservativity}: there exists a $2$-functor $\II_{\mathbf{F}} : \mathbf{F}^{\mathrm{op}} \to \Sint$ satisfying properties (I-1)--(I-4) of Theorem~\ref{thm:I-existence}, and moreover the pair $(\Cl_{\mathbf{F}}, \II_{\mathbf{F}})$ is \emph{jointly conservative}: two parallel $1$-morphisms of $\mathbf{F}$ that are identified both by $\Cl_{\mathbf{F}}$ and by $\II_{\mathbf{F}}$ are equal (up to invertible $2$-cell). The canonical classifier $\mathbf{F}_S^*$ of Theorem~\ref{thm:meta-cat} satisfies this clause by construction (its morphisms are pairs of a base morphism and an intensional-fiber morphism), so the clause does not empty the class.
\end{itemize}
...
\end{itemize}
Non-equivalence is asserted in $\Meta_{\mathrm{Cls}}$ \emph{over} $\fM$: morphisms of meta-structures are required to commute with the classification functors (up to invertible $2$-cell). If any two of the three were $2$-equivalent over $\fM$, their classification images would coincide as sub-stacks of $\fM$, contradicting these observations. (As bare $2$-categories, abstract equivalences ignoring $\Cl$ are not excluded and are irrelevant to the plurality claim.)
...
Let $\mathcal{R}_\cE \subseteq \Mor_1(\cE)$ be the class of \emph{reflection morphisms}: $1$-morphisms of the form $(\tau, \Phi)$ in which $\tau$ is a gauge transformation in $\cF$ (Convention~\ref{conv:notation}) and the composite $r_2 \circ \Phi \circ \iota_1 : \Syn(F_1) \to \Syn(F_2)$ is an $(\infty, n)$-equivalence. The class $\mathcal{R}_\cE$ contains all identities and all $1$-equivalences, is closed under composition (reflectors compose along the invertible counits), is stable under the $2$-pullbacks existing in $\cE$ (componentwise, since gauge transformations and equivalences are pullback-stable), and is saturated; hence it satisfies the calculus-of-fractions conditions (BF1)--(BF5) of Lemma~\ref{lem:pronk}. Define
\[ \mathrm{gauge}_\cE := \text{the $2$-localization class generated by } \mathcal{R}_\cE, \qquad \fM_\cE := \cE[\mathcal{R}_\cE^{-1}] \]
via Pronk's bicategory of fractions (Lemma~\ref{lem:pronk}). Two enactments are \emph{gauge$_\cE$-equivalent} iff they are connected by a zig-zag of reflection morphisms. In particular the reflection $(\id_F, r) : (F, \cC, \iota, r) \to \varepsilon(F)$ is inverted in $\fM_\cE$ --- this, and only this, is what ``passing to gauge'' adds: the unit $\eta_r$ of a proper reflection is \emph{not} invertible in $\cE$ itself.
\end{definition}
...
\begin{proof}
\emph{(a) Full faithfulness of $\varepsilon$.} Let $F_1, F_2 \in \cF$. Unpacking Definition~\ref{def:enactments}, a $1$-morphism $\varepsilon(F_1) \to \varepsilon(F_2)$ is a pair $(\phi, \Phi)$ with $\phi : F_1 \to F_2$ in $\cF$ and $\Phi : \Syn(F_1) \to \Syn(F_2)$ an $S$-preserving $(\infty, n)$-functor equipped with an invertible $2$-cell $\Phi \Rightarrow \Syn(\phi)$. The \emph{classifying property} of syntactic categories (functorial semantics: $S$-preserving $(\infty, n)$-functors $\Syn(F_1) \to \cC$ correspond naturally to $F_1$-model structures on $\cC$; Lambek--Scott~\cite{LambekScott1986} for $n = 1$, Kapulkin--Lumsdaine~\cite{KapulkinLumsdaine2021} for the type-theoretic $(\infty, 1)$-case), applied with $\cC = \Syn(F_2)$, identifies $S$-preserving functors $\Syn(F_1) \to \Syn(F_2)$ with $F_1$-models in $\Syn(F_2)$, i.e.\ with interpretations $\phi : F_1 \to F_2$: every such functor arises, up to an invertible $2$-cell, as $\Syn(\phi)$ for a unique $\phi$. Hence the forgetful map of hom-$2$-categories
\[
...
\emph{(b) Essential surjectivity at gauge level.} Let $(F, \cC, \iota, r) \in \cE$. The reflection morphism $(\id_F, r) : (F, \cC, \iota, r) \to \varepsilon(F) = (F, \Syn(F), \id, \id)$ --- whose structure cell is the invertible counit $\varepsilon_r : r \circ \iota \Rightarrow \id_{\Syn(F)}$ (left adjoint to a fully faithful functor; Riehl--Verity~\cite{RiehlVerity2022} Prop.~2.1.11) --- lies in the class $\mathcal{R}_\cE$ of Definition~\ref{def:gauge-E} (here $r_2 \circ \Phi \circ \iota_1 = \id \circ r \circ \iota \simeq \id_{\Syn(F)}$). By construction of $\fM_\cE = \cE[\mathcal{R}_\cE^{-1}]$, this morphism becomes invertible in $\fM_\cE$, yielding a canonical gauge$_\cE$-equivalence
\[
  (F, \cC, \iota, r) \;\simeq_{\mathrm{gauge}_\cE}\; \varepsilon(F) \quad\text{in } \fM_\cE.
\]
We emphasise what is \emph{not} claimed: the unit $\eta_r : \id_\cC \Rightarrow \iota \circ r$ is not invertible in $\cE$ unless $\iota$ is essentially surjective; the equivalence above is created by the localization, exactly as ring-Morita equivalence is created by inverting bimodule equivalences. Uniqueness of $r$ up to unique invertible $2$-cell given $\iota$ makes the equivalence natural in the object. Hence $\varepsilon$ is essentially surjective after passing to gauge-classes, and the theorem acquires its precise content: \emph{the enactment moduli, localized at reflections, is equivalent to the theory moduli}.
...
The extension of the MSFS framework by $\cE$ and the structure maps $(\alpha, \varepsilon)$ is a conservative extension of the $\cF$-based framework: every statement about $\fM_\cE$ is equivalent, via Theorem~\ref{thm:ac-oc-duality}, to a statement about $\fM$. In particular the ambient metatheory needed to construct and reason about $\cE$ is the same as for $\cF$:
\[
  \Con(\text{MSFS with } \cE) \;=\; \Con(\text{MSFS}) \;=\; \Con(\ZFC + 2\text{-inacc}).
\]
\end{corollary}
...
Let $\cC$ be an $(\infty, \infty)$-cartesian closed category admitting an object $A$ and a $1$-morphism $e : A \to Y^A$ that is \emph{weakly point-surjective up to coherent homotopy} (every global point $1 \to Y^A$ factors through $e$ up to invertible higher cells). Then every $f : Y \to Y$ has an $(\infty, \infty)$-coherent fixed point.
\end{lemma}
...
Write $(\infty, \infty)\text{-}\Cat := \colim_{n < \infty} (\infty, n)\text{-}\Cat$ along the canonical inclusions, and define $(\infty, \infty + 1)\text{-}\Cat$ as the $(\infty, \infty)$-category of categories enriched in $(\infty, \infty)\text{-}\Cat$ (one further enrichment step). Then the canonical inclusion $(\infty, \infty)\text{-}\Cat \hookrightarrow (\infty, \infty + 1)\text{-}\Cat$ is an equivalence: the colimit is idempotent under enrichment.
\end{theorem}

\begin{proof}[Proof (assembly)]
At each finite level, categories enriched in $(\infty, n)\text{-}\Cat$ are $(\infty, n+1)$-categories, uniquely up to equivalence by the unicity theorem of Barwick--Schommer-Pries~\cite{BarwickSchommerPries2021} with the model comparisons of Bergner--Rezk~\cite{BergnerRezk2013}. Enrichment commutes with the filtered colimit over $n$, so $(\infty, \infty+1)\text{-}\Cat \simeq \colim_n (\infty, n+1)\text{-}\Cat = (\infty, \infty)\text{-}\Cat$; the globular model of Ara--Maltsiniotis~\cite{AraMaltsiniotis2017} realizes the same idempotence for $\omega$-categories. This ``unbounded-depth'' reading matches the convention fixed in (R5); no statement in the paper requires the alternative everywhere-non-invertible reading.
\end{proof}
